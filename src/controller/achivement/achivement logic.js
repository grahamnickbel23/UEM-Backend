...
export default class achivementController {
    ...
    static async docDelete(req, res) {
        ...
        // Remove the preview from userSchema array
        const achievementDocId = doesDocExisit._id?.toString();

        const updatedUserInfo = await userSchema.findByIdAndUpdate(
            userId,
            { $pull: { achivementSchema: { url: achievementDocId } } },
            { new: true }
        );
    ...
    // list all deleted achievements
    static async getAllDeletedAchievements(req, res) {

        // find all soft-deleted achievements
        const deletedAchievements = await achivementSchema.find(
            { isDeleted: true },
            "title organizer _id"
        ).lean();

        // if none found
        genaralResponse.genaral404Error(!deletedAchievements, "No deleted achievements found", res)

        // format response (add url field = _id)
        const formatted = deletedAchievements.map(item => ({
            title: item.title,
        ...
        // create a log
        logger.info(`${req.requestId} listing deleted doc was sucessful`)

        // send response
        return res.status(200).json({
            success: true,
            message: "Deleted achievements fetched successfully",
            data: formatted
        });
